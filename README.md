...
- **页面风格（默认 `guizang` 主题，2026-09-08 起日式黑白版）**：**白底、黑白灰文字、加粗宋体 / 明朝体大标题、细分隔线与大留白**；阅读区域居中，最大宽度 **760px**，手机两侧 **24px** 边距。标题「章鱼 AI·全景分析 —AI 港股研报」（刊头 **34px 粗宋体**，栏目标题 **26px 粗宋体**），正文 **16px**、长段落行高 **1.85**。刊头使用 64px 章鱼图标 + 全部栏目手绘图标横排（多图标显示），栏目以 40px 大图标点缀，均通过 **Koboyo 官方 HTTPS SVG 直链** `<img src="https://koboyo.com/icons/svg/…svg">` 引用，**不下载、不内嵌、不代理**；只用系统字体，不加载远程字体。保留全部行情、分析、来源与新鲜度，涨跌改用 **▲ 涨 / ▼ 跌 / ■ 平** 和正负号区分，不再依赖红绿颜色。继续使用纯内联样式、单列表格与 `bgcolor` 双写，无外部 CSS / JavaScript，允许缩放。
