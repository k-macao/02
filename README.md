...
- **📊 AI 新闻情绪因子**（规则合成）：对**近 72 小时标题窗口**（含跨运行存档 `output/news_history.json`，
  截至运行时刻 / 报告日 23:59:59）内的资讯标题逐条词表评分（S，附命中词），按热门榜单个股名
  归因，输出 **DNS**=(正−负)/总数（近 72h 窗口口径，页面标注「72h 情绪」）、**MOM 情绪动量**=近 3 有
  评分日均−近 20 有评分日均、**ANV 异常新闻量**=今日条数 vs 近 30 天均值±σ（z 值，>均值+2σ 标异常
  放量；MOM / ANV 仍按自然日——「今日」= 报告日当天的标题桶）。跨日基线存
  `output/sentiment_history.json`（随日报提交，幂等更新）；冷启动 / 样本不足明确标注；窗口内有标题但
  均未点名榜单个股时，栏目显示「样本不足」占位（不整栏消失），窗口内无标题时才缺席。渲染位置在
  各资讯栏目（全球头条 / 东财快讯 / A股市场 / 港股名家频道）之后、流动性分析之前。无需大模型 API、
  可复现、不伪造内容，明确标注「非投资建议」。
