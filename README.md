...
- **页面风格（默认 `guizang` 主题，2026-09-08 起日式黑白版）**：**白底、黑白灰文字、加粗宋体 / 明朝体大标题、细分隔线与大留白**；阅读区域居中，最大宽度 **760px**，手机两侧 **24px** 边距。标题「章鱼 AI·全景分析 —AI 深度研报」（刊头 **56px 粗宋体**，栏目标题 **44px 粗宋体**），正文 **10px**、元信息 **9px**、长段落行高 **1.85**；行情价格 / 市场倾向等个别突出数字 **36–44px**。刊头使用 **128px** 章鱼图标 + 全部栏目手绘图标横排（**72px**，多图标显示），栏目以 **96px** 极大图标点缀（落在标题上方），均通过 **Koboyo 官方 HTTPS SVG 直链** `<img src="https://koboyo.com/icons/svg/…svg">` 引用，**不下载、不内嵌、不代理**；只用系统字体，不加载远程字体。保留全部行情、分析、来源与新鲜度，涨跌改用 **▲ 涨 / ▼ 跌 / ■ 平** 和正负号区分，不再依赖红绿颜色。行情 / 全景 / 情绪总览 / 政策冲击 / 数据审计等结构化数据用键值表或多列表格整合（资讯长文与 AI 研判仍用卡片）。继续使用纯内联样式、单列满宽表格与 `bgcolor` 双写，无外部 CSS / JavaScript，允许缩放。
...
- **🏛️ 国家政策官方网站**：直接抓取国务院办公厅主办的**中国政府网「最新政策」**页面，
  只接受 `gov.cn/zhengce/` 政策正文链接；每条保留官方发布日期与原文 URL，作为政策因子的独立输入。
  政府网页面不可用时明确标注「数据暂缺」，不以搜索结果、RSSHub 或媒体转载冒充官方发布；
...
- **📊 AI 新闻情绪因子**（规则合成）：按**最近成交日 A股 / 港股 / 美股 成交量前五**（热门榜单
  ...
  **ANV 异常新闻量**=今日条数 vs 近 30 天均值±σ（z 值，>均值+2σ 标异常放量；MOM / ANV 仍按
  ...
  港股名家频道）之后。无需大模型 API、可复现、不伪造内容，明确标注「非投资建议」。
- **🏛️ 政策因子**（规则合成）：抓取后、推送前单独构建，推送页首位渲染。对**近 15 个自然日窗口**
  ...
  数据化总结（口径标注「近 15 日窗口」）。其中中国政府网直接发布的条例、规划、办法等条目即使
  没有方向性触发词，也会以「政策发布」中性维度纳入，并在「政策新闻逐条」保留**官方原文链接**；
  触发词被否定修饰时跳过；窗口内零政策 / 宏观新闻时栏目缺席。无需大模型 API、可复现、不伪造内容，
  明确标注「非投资建议」。
- **页面栏目固定阅读顺序**（2026-09-09 起）：政策因子 → AI 盘研判（导读）→ 行情速览 → A股大盘
  全景复盘 → 全球头条 → 东方财富快讯 → A股市场 → 港股名家频道 → AI 新闻情绪因子 → 成交量与
  AI 新闻情绪因子 → 本次数据可用性（审计）。无数据的栏目自动缺席，不打乱其余顺序。
...
> `OCTOPUS_NATIONAL_POLICY_TOP_N` 可调整单次抓取上限（默认 30）。
>
