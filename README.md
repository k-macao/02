...
- **📊 AI 新闻情绪因子**（规则合成）：对当天资讯标题逐条词表评分（S，附命中词），按热门榜单个股名
  ...
  **ANV 异常新闻量**=今日条数 vs 近 30 天均值±σ（z 值，>均值+2σ 标异常放量）。跨日基线存
  `output/sentiment_history.json`（随日报提交，幂等更新）；冷启动 / 样本不足明确标注；当天标题存在但
  均未点名榜单个股时，栏目显示「样本不足」占位（不整栏消失），全天无当天标题时才缺席。渲染位置在
...
- **🏛️ 政策因子**（规则合成）：抓取后、推送前单独构建，推送页首位渲染。对当天资讯标题做政策维度
  ...
  数据化总结。触发词被否定修饰时跳过；零政策 / 宏观新闻时栏目缺席。无需大模型 API、可复现、
  不伪造内容，明确标注「非投资建议」。
