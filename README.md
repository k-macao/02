...
- **页面风格：复古游戏像素风（Retro Pixel Market Quest）**——暗色街机终端底、霓虹青 / 电光蓝 / 像素黄 / 品红高对比配色、纯直角硬描边和实体投影；栏目以 `LVL` 关卡呈现，并配有独立的 44px 大图标砖与纯 HTML 8-bit 章鱼图标。**涨跌使用颜色 + `▲ 涨` / `▼ 跌` / `■ 平` 三重编码**，行情、榜单与流动性锚点均显示高对比涨跌徽标；**AI 盘研判置于首屏，以 AI CORE 主控卡、方向 / 信号分 / 置信度计分板和 14px「AI 主结论」高亮框凸显**，板块、技术、风险与关注清单各自使用独立像素面板。成交量榜单不再单独成栏，只保留 AI 研判结果；港股名家频道保留聊天记录式消息结构。全部为内联样式 + 表格布局，适配微信 / PushPlus 渲染；
...
- **🔥 热门榜单**（A股 / 港股 / 美股 成交量前五，东方财富 push2 免费接口）：**不再单独渲染三个榜单栏目**，
  原始榜单数据仅作为 **AI 盘研判**（「N 个市场榜单活跃」结论、板块热度、关注清单）与
  **AI 量化流动性报告**（成交额锚点）的研判输入，页面只保留 AI 对三个榜单成交量的研判结果；
