...
- **页面风格（默认 `guizang` 主题，2026-09-08 起日式黑白版）**：**白底、黑白灰文字、加粗宋体 / 明朝体大标题、细分隔线与大留白**；阅读区域居中，最大宽度 **760px**，手机两侧 **24px** 边距。标题「章鱼 AI·全景分析 —AI 深度研报」（刊头 **56px 粗宋体**，栏目标题 **44px 粗宋体**），正文 **10px**、元信息 **9px**、长段落行高 **1.85**；行情价格 / 流动性评分 / 市场倾向等个别突出数字 **36–44px**。刊头使用 **128px** 章鱼图标 + 全部栏目手绘图标横排（**72px**，多图标显示），栏目以 **96px** 极大图标点缀（落在标题上方），均通过 **Koboyo 官方 HTTPS SVG 直链** `<img src="https://koboyo.com/icons/svg/…svg">` 引用，**不下载、不内嵌、不代理**；只用系统字体，不加载远程字体。保留全部行情、分析、来源与新鲜度，涨跌改用 **▲ 涨 / ▼ 跌 / ■ 平** 和正负号区分，不再依赖红绿颜色。行情 / 全景 / 流动性指标 / 情绪总览 / 政策冲击 / 数据审计等结构化数据用键值表或多列表格整合（资讯长文与 AI 研判仍用卡片）。继续使用纯内联样式、单列满宽表格与 `bgcolor` 双写，无外部 CSS / JavaScript，允许缩放。
...
- **推送模式**：2026-09-09 起默认改用 **PushPlus「一对多」群组推送**，群组编码
  `oai.1`（日报、检验未通过告警、推送失败告警都发往该群组）；如需切换回来一份对
  自己发送，设置 `PUSHPLUS_TOPIC=""`；如需改群组，设置环境变量
...
- **🧭 A股大盘全景复盘**（2026-09-08 新增，东方财富 push2 / push2his 免费接口）：一页总览当日 A股全貌——
  ...
  **④ 南北向资金**（港交所 2024-08-19 起停止披露实时 / 每日净买入额；本页按东财沪深港通历史报表
  `RPT_MUTUAL_DEAL_HISTORY` 的北向合计（005）/ 南向合计（006）成交总额，按最近一个完整交易日（前一收盘）
  展示北向、南向成交总额，并附披露口径说明，绝不编造净买入数字）；
...
- **📊 AI 新闻情绪因子**（规则合成）：按**最近成交日 A股 / 港股 / 美股 成交量前五**（热门榜单
  `HOT_STOCK_TOP_N`）逐股输出 **AI 新闻情绪分**与**总结评论（含原因）**。对**近 72 小时标题窗口**
  （含跨运行存档 `output/news_history.json`，截至运行时刻 / 报告日 23:59:59）内的资讯标题逐条
  词表评分（S，附命中词），按热门榜单个股名归因，输出 **DNS**=(正−负)/总数（近 72h 窗口口径，
  页面标注「72h 情绪」）、**MOM 情绪动量**=近 3 有评分日均−近 20 有评分日均、
  **ANV 异常新闻量**=今日条数 vs 近 30 天均值±σ（z 值，>均值+2σ 标异常放量；MOM / ANV 仍按
  自然日——「今日」= 报告日当天的标题桶）。每只上榜股的「总结评论 + 原因」由命中标题、词表命中词、
  动量与新闻量规则生成；窗口内无相关新闻的上榜股明确显示「暂无评分 + 原因」，绝不凭价格涨跌反推
  新闻情绪。跨日基线存 `output/sentiment_history.json`（随日报提交，幂等更新）；冷启动 / 样本不足
  明确标注；热门榜单缺席时栏目缺席。渲染位置在各资讯栏目（全球头条 / 东财快讯 / A股市场 /
  港股名家频道）之后、流动性分析之前。无需大模型 API、可复现、不伪造内容，明确标注「非投资建议」。
- **🏛️ 政策因子**（规则合成）：抓取后、推送前单独构建，推送页首位渲染。对**近 15 个自然日窗口**
  （含报告日；依赖跨运行存档 `output/news_history.json`）内的政策 / 宏观资讯标题做政策维度
  识别（货币 / 监管 / 扶持 / 财政 / 地产 / 开放 / 贸易 / **宏观数据**——含 CPI / PPI / 社融 /
  统计局等经济数据口径），经关键词矩阵映射到行业受益 / 受损权重，
  汇总为 **PolicyShockIndex**（行业 PSI 与大盘 PSI，>0 偏暖 / <0 偏冷 / =0 中性），附规则生成的
  数据化总结（口径标注「近 15 日窗口」）。触发词被否定修饰时跳过；窗口内零政策 / 宏观新闻时栏目
  缺席。无需大模型 API、可复现、不伪造内容，明确标注「非投资建议」。
- **页面栏目固定阅读顺序**（2026-09-09 起）：政策因子 → AI 盘研判（导读）→ 行情速览 → A股大盘
  全景复盘 → 全球头条 → 东方财富快讯 → A股市场 → 港股名家频道 → AI 新闻情绪因子 → 成交量与
  流动性分析 → 本次数据可用性（审计）。无数据的栏目自动缺席，不打乱其余顺序。
...
PUSHPLUS_TOPIC=oai.1 python3 output/push.py     # 一对多群组推送（默认已是 oai.1；置空可回退一对一）
```
